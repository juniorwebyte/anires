...
import { MemeEffectsProvider, useMemeEffects } from "@/components/meme-effects-controller"
import EffectsPanel from "@/components/effects-panel"

// Implementação alternativa para reprodução de áudio
// Usando uma abordagem mais simples e robusta
const audioContext = typeof window !== "undefined" ? new (window.AudioContext || window.webkitAudioContext)() : null
const audioBuffer = null

// Função para carregar e reproduzir áudio usando Web Audio API
const playSound = (soundType = "burro") => {
  if (typeof window === "undefined" || !audioContext) return

  try {
    // Criar um oscilador simples para gerar um som de "beep" como fallback
    // Isso garante que algum som será reproduzido mesmo se o arquivo não existir
    const oscillator = audioContext.createOscillator()
    const gainNode = audioContext.createGain()

    oscillator.type = "sine"
    oscillator.frequency.setValueAtTime(soundType === "burro" ? 300 : 500, audioContext.currentTime)
    ...
    console.log(`Som "${soundType}" reproduzido com sucesso (fallback)`)
  } catch (error) {
...
const MemeCard = ({ src, alt, title, description }) => {
  ...
  const handleShare = () => {
    ...
    if (typeof window !== "undefined" && navigator.share) {
      ...
      toast({
        title: "Compartilhar",
        description: "Função de compartilhamento não disponível neste navegador",
      })
    }
  }
  ...
  const handleMouseMove = (e) => {
    if (isHovered && Math.random() < 0.05 && cardRef.current) {
        ...
        {!isFlipped ? (
          ...
          >
            <div className="relative">
              <motion.img
                src={trollImage || "/placeholder.svg?height=500&width=500"}
                alt={alt}
...
export default function MemesSecretos() {
  ...
  const { toast } = useToast()
  const { enableEffects, activateEffect } = useMemeEffects()
  ...
  // Implementação simplificada para música de fundo usando Web Audio API
  const toggleBackgroundMusic = () => {
    ...
    if (isMusicPlaying) {
      ...
      // Notificar o usuário
      toast({
        title: "Música desativada",
        description: "A música foi desativada com sucesso.",
        variant: "default",
      })
    } else {
      // Iniciar a música (usando osciladores para criar um som simples)
      try {
        // Resumir o contexto de áudio se estiver suspenso
        if (audioContext.state === "suspended") {
          audioContext.resume()
        }

        // Criar um oscilador para simular música
        const oscillator = audioContext.createOscillator()
        const gainNode = audioContext.createGain()
        ...
        // Notificar o usuário
        toast({
          title: "Música ativada",
          description: "A música foi ativada com sucesso.",
          variant: "default",
          className: "bg-gradient-to-r from-purple-600 to-blue-600",
        })
      } catch (error) {
        console.error("Erro ao ativar música:", error)

        toast({
          title: "Erro",
          description: "Não foi possível ativar a música. Tente novamente.",
          variant: "destructive",
        })
      }
    }
  ...
  // Função para ativar efeitos especiais
  const handleActivateEffects = useCallback(() => {
    // Ativar os efeitos
    enableEffects()

    // Ativar um efeito aleatório para demonstração
    const effects = ["explosion", "rainbow", "matrix", "earthquake", "burroRain", "colorCycle", "glitch", "disco"]
    const randomEffect = effects[Math.floor(Math.random() * effects.length)]

    activateEffect(randomEffect)

    toast({
      title: "Efeitos ativados! 🎉",
      description: "Você desbloqueou efeitos especiais! Clique no botão flutuante para mais opções.",
      variant: "default",
      className: "bg-gradient-to-r from-purple-600 to-blue-600",
    })
  }, [activateEffect, enableEffects, toast])
